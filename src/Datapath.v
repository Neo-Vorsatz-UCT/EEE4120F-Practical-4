...
module Datapath (
    ...
    // Initialise pc_current to 16'd0 in an initial block.
    initial begin
        pc_current <= 16'd0;
    end

    // Update pc_current to pc_next on every positive clock edge.
    always @(posedge clk) begin
        pc_current <= pc_next;
    end

    // Compute pc2 = pc_current + 16'd2 using a continuous assignment.
    ...
    // Instantiate the InstructionMemory module using named port connections.
    InstructionMemory im (
        .pc          (pc_current),
        .instruction (instr)
    );

    // Drive the opcode output from the fetched instruction:
    ...
    // Select the write-back register address based on the RegDst control.
    ...
    // Assign the read addresses from the instruction fields:
    ...
    // Instantiate the GPR module using named port connections.
    GPR reg_file (
        .clk              (clk),
    ...
    // Implement all of the above using continuous assignments.
